...
class RefresTokens{
     ...
     *
     * @name .find()
     * @param {string} `refreshToken` - details about the user.
     * @return {Promise}] a promise to perform async operations with. The result of the promise is the record that
     * was added
     */
    find(refreshToken){
        return this._tokens.findOne({token: refreshToken}).exec();
    }

    /**
     * removes a record from the collection, based on the token
     * @name .delete()
     * @param refreshToken {string} - the token to delete
     * @return {Promise}] a promise to perform async operations with. The result of the promise is the record that
     * was added
     */
    delete(refreshToken) {
        return this._tokens.remove({token: refreshToken}).exec();
    }
